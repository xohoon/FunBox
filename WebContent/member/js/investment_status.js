...
$(function() {
	 ...
	 * $('.list_show').offset().top; var liOffs = liOff*1+56 if(winW>1200){
	 * $('.list').height(h); }else{ $('.list').height(400);
	 * $('.list').offset({top:liOffs}) }
	 */
});

/*
 * $('.list_show').on('click',function(){ $('.list').toggleClass('on'); if(
...
/*	$("#cf_financial_selector").change(
			function() {
				cf_financial_download.attr("href",
						"./CompanyFileDownload?filename="
								+ $("#cf_financial_selector").val()
